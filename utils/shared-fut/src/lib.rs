...
// SPDX-FileCopyrightText: 2025 2016 Alex Crichton
// SPDX-FileCopyrightText: 2025 2017 The Tokio Authors
// SPDX-FileCopyrightText: 2025 2025 Foundation Devices, Inc. <hello@foundation.xyz>
...
impl<Fut: Future> fmt::Debug for Shared<Fut> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Shared").field("inner", &self.inner).field("waker_key", &self.waker_key).finish()
    }
}

impl<Fut: Future> fmt::Debug for Inner<Fut> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("Inner").finish() }
}

impl<Fut: Future> fmt::Debug for WeakShared<Fut> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.debug_struct("WeakShared").finish() }
}
    ...
    Fut: Future,
{
    ...
    pub fn strong_count(&self) -> Option<usize> { self.inner.as_ref().map(Arc::strong_count) }
    ...
    /// and acting on the result.
    pub fn weak_count(&self) -> Option<usize> { self.inner.as_ref().map(Arc::weak_count) }
...
where
    ...
    Fut::Output: Clone,
{
    /// Returns `true` if this shared future has completed.
    pub fn is_terminated(&self) -> bool { self.inner.is_none() }
}
    ...
    Fut::Output: Clone,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match inner.notifier.state.compare_exchange(IDLE, POLLING, SeqCst, SeqCst).unwrap_or_else(|x| x) {
            IDLE => {
...
where
    Fut: Future,
{
    fn clone(&self) -> Self { Self { inner: self.inner.clone(), waker_key: NULL_WAKER_KEY } }
}
...
/// A custom waker implementation for the notifier.
struct NotifierWaker {
    notifier: Arc<Notifier>,
}

impl std::task::Wake for NotifierWaker {
    fn wake(self: Arc<Self>) { self.wake_by_ref() }

    fn wake_by_ref(self: &Arc<Self>) {
        let wakers = &mut *self.notifier.wakers.lock().unwrap();
...
/// Create a waker reference from a notifier.
fn waker_ref(notifier: &Arc<Notifier>) -> std::task::Waker {
    let waker = Arc::new(NotifierWaker { notifier: notifier.clone() });
    std::task::Waker::from(waker)
}
...
mod tests {
    ...
    fn test_shared_basic() {
        let shared = Shared::new(ready(42));
        let mut cx = std::task::Context::from_waker(std::task::Waker::noop());

        let mut shared1 = shared.clone();
        let poll1 = Pin::new(&mut shared1).poll(&mut cx);
        assert_eq!(poll1, Poll::Ready(42));

        // Second poll should panic - we test this by checking that the inner is None
        let mut shared2 = shared;
        assert!(shared2.inner.is_some());
        let _ = Pin::new(&mut shared2).poll(&mut cx);
        assert!(shared2.inner.is_none());
    }
    ...
    fn test_peek() {
        ...
        let mut cx = std::task::Context::from_waker(std::task::Waker::noop());
        let mut shared1 = shared.clone();
        let _ = Pin::new(&mut shared1).poll(&mut cx);

        assert_eq!(shared.peek(), Some(&42));
    }
