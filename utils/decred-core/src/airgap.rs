...
pub struct SignRequest {
    ...
    /// OPTIONAL (added post-interop, format stays v1 - the field is
    /// additive and old 7-element packages decode with None): BIP32
    /// fingerprint of the ACCOUNT xpub this request was built against
    /// (first 4 bytes of hash160 of the account's compressed pubkey).
    /// Lets the device detect "wrong wallet open" with a friendly message
    /// instead of a late script mismatch. Never required; never a
    /// security control (ScriptMismatch remains the fund protector).
    #[n(7)]
...
mod fp_compat_tests {
    ...
    /// 7-element package (pre-fingerprint, generated by the original Pulse
    /// interop format). Must decode with account_fp = None.
    ...
    /// Same package with an appended fingerprint [1,2,3,4].
    ...
    #[test]
    fn legacy_seven_element_decodes_with_none() {
        let req = decode_sign_request(&hex(LEGACY7)).expect("legacy must decode");
        assert_eq!(req.account_fp, None);
    ...
    #[test]
    fn eight_element_decodes_with_fingerprint() {
        let req = decode_sign_request(&hex(WITHFP)).expect("v1+fp must decode");
        assert_eq!(req.account_fp, Some([1, 2, 3, 4]));
        req.validate().expect("fixture is economically valid");
    }

    #[test]
    fn encode_roundtrips_both_ways() {
        let mut req = decode_sign_request(&hex(WITHFP)).unwrap();
        let bytes = encode_sign_request(&req).unwrap();
        assert_eq!(decode_sign_request(&bytes).unwrap().account_fp, Some([1, 2, 3, 4]));
        req.account_fp = None;
        let bytes = encode_sign_request(&req).unwrap();
        assert_eq!(decode_sign_request(&bytes).unwrap().account_fp, None);
    }
}
