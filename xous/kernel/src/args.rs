...
static mut KERNEL_ARGUMENTS_BASE: *const u32 = core::ptr::null::<u32>();
...
impl KernelArguments {
    ...
    /// Get the size of the entire kernel argument structure, including additional data
    pub fn total_size(&self) -> usize { unsafe { self.base.add(3).read() as usize * 4 } }
}
...
impl KernelArgument {
    pub fn new(base: *const u32, offset: usize) -> Self {
        let name = unsafe { base.add(offset / 4).read() };
