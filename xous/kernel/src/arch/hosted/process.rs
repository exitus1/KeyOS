...
use xous::{AppId, ProcessInit, ProcessStartup, ThreadInit, PID, TID};
...
pub fn register_connection_for_key(mut conn: TcpStream, key: AppId) -> Result<PID, xous::Error> {
    PROCESS_TABLE.with(|pt| {
...
impl Process {
    ...
    pub fn set_tid(&mut self, thread: TID) -> Result<(), xous::Error> {
        assert!(thread > 0);
        PROCESS_TABLE.with(|pt| {
            let mut process_table = pt.borrow_mut();
    ...
    /// Initialize this process with the given memory space.
    pub fn create(
        pid: PID,
        ...
        PROCESS_TABLE.with(|process_table| {
            ...
            process.threads[INITIAL_TID - 1].allocated = true;
