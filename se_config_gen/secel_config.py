...
if MPY:
    ...
    # full version for desktop... uses "dataclasses" module to good effect
    ...
    def make_bitmask(name, defs):
        ...
        '''
        rv = make_dataclass(name, ((n, typing.Any, field(default=0)) for w, n in defs))
...
class ComboConfig(object):
    ...
    def ec_key(self, limited_sign=False, priv_write=False):
        # basics for an EC key
        self.kc.KeyType = 4         # secp256r1
        self.kc.Private = 1         # is an EC private key
        self.kc.Lockable = 0        # normally set in stone
        self.kc.PubInfo = 1         # pubkey can always be generated
        ...
        self.sc.WriteConfig = 0x6 if priv_write else 0x2   # enable GenKey (and also PrivWrite if requested), no mac for key roll
        return self

    def ec_pubkey(self, write_kn):
        # basics for an EC key
        self.kc.KeyType = 4         # secp256r1
        self.kc.Private = 0         # is an EC pubkey
        self.kc.Lockable = 0        # normally set in stone
        self.sc.IsSecret = 0        # freely readable
        self.sc.ReadKey = 0xf       # allow all usages
        self.sc.WriteConfig = 0x4
    ...
    def no_write(self):
        self.sc.IsSecret = 1        # because is a secret key
        self.sc.WriteKey = 0xf
        self.sc.WriteConfig = 0x08  # Can't write
        return self
