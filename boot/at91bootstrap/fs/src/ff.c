...
)
{
	...
	*rfs = fs = FatFs[vol];			/* Return pointer to the corresponding file system object */
	...
	if (fs->fs_type) {			/* If the logical drive has been mounted */
		...
		if (stat & STA_INIT) {	/* and the physical drive is kept initialized (has not been changed), */
	...
	if (!(stat & STA_INIT))				/* Check if the initialization succeeded */
...
)
{
	...
	if (!(disk_status(fs->drv) & STA_INIT))
		return FR_NOT_READY;
...
)
{
	...
	if (!(stat & STA_INIT)) return FR_NOT_READY;
