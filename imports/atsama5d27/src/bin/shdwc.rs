...
fn _entry() -> ! {
    ...
    pmc.enable_peripheral_clock(PeripheralId::Rxlp);
    ...
    writeln!(uart, "Shutting down in 5 seconds").ok();
    pit.busy_wait_ms(MASTER_CLOCK_SPEED, 5000);

    let mut rxlp = Rxlp::new();
    ...
    let c = rxlp.read();
    writeln!(uart, "Received character: 0x{:02x}", c).ok();
    writeln!(uart, "shdwc status: {:?}", shdwc.status()).ok();
