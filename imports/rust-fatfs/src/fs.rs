...
#[must_use = "use .unmount() to make sure disk is unmounted"]
...
impl<T: ReadWriteSeek> FileSystem<T> {
    ...
    pub fn root_dir_first_cluster(&self) -> u32 {
        *self.bpb.root_dir_first_cluster.borrow()
    }

    pub fn set_root_dir_first_cluster(&self, cluster: u32) -> io::Result<()> {
        assert_eq!(self.fat_type, FatType::Fat32);
        const OFFSET: u64 = 0x2C;
        let mut disk = self.disk.borrow_mut();
        // update boot sector
        disk.seek(SeekFrom::Start(OFFSET))?;
        disk.write_u32::<LittleEndian>(cluster)?;
        // update backup boot sector
        disk.seek(SeekFrom::Start(self.bpb.bytes_from_sectors(self.bpb.backup_boot_sector()) + OFFSET))?;
        disk.write_u32::<LittleEndian>(cluster)?;
        disk.flush()?;
        *self.bpb.root_dir_first_cluster.borrow_mut() = cluster;
        Ok(())
    }
    ...
    /// Note: This function returns label stored in the BPB structure. Use `read_volume_label_from_root_dir` to read
    ...
    pub fn root_dir<'b>(&'b self) -> Dir<'b, T> {
        ...
        let root_rdr = {
            match self.fat_type {
                ...
                _ => DirRawStream::File(File::new(Some(*self.bpb.root_dir_first_cluster.borrow()), None, self)),
            }
        };
    ...
    pub fn total_clusters(&self) -> u32 { self.total_clusters }
    ...
    pub fn fat_slice<'b>(&'b self) -> DiskSlice<FsIoAdapter<'b, T>> {
        let io = FsIoAdapter { fs: self };
        fat_slice(io, &self.bpb)
    }

    pub fn fs_io_adapter(&self) -> FsIoAdapter<'_, T> { FsIoAdapter { fs: self } }
    ...
    pub fn free_cluster(&self, cluster: u32) -> io::Result<()> {
        let mut fat = self.fat_slice();
        crate::table::write_fat(&mut fat, self.fat_type, cluster, crate::table::FatValue::Free)?;
        let mut fs_info = self.fs_info.borrow_mut();
        fs_info.add_free_clusters(1);
        Ok(())
    }

    pub fn alloc_cluster(&self, prev_cluster: Option<u32>, zero: bool) -> io::Result<u32> {
        trace!("alloc_cluster");
    ...
    pub fn unmount(self) -> io::Result<T> {
    ...
    pub fn with_disk<R>(&self, f: impl FnOnce(&mut T) -> R) -> R {
        let mut disk = self.disk.borrow_mut();
        f(&mut disk)
    }
}
...
pub(crate) fn write_zeros<T: ReadWriteSeek>(mut disk: T, mut len: u64) -> io::Result<()> {
    #[repr(align(0x1000))]
    struct AlignedZeros([u8; 0x8000]);
    const ZEROS: AlignedZeros = AlignedZeros([0u8; 0x8000]);
    while len > 0 {
        let write_size = cmp::min(len, ZEROS.0.len() as u64) as usize;
        disk.write_all(&ZEROS.0[..write_size])?;
...
pub fn format_volume<T: ReadWriteSeek>(mut disk: T, options: FormatVolumeOptions) -> io::Result<()> {
    ...
    if fat_type == FatType::Fat32 {
        ...
        assert!(root_dir_first_cluster == *boot.bpb.root_dir_first_cluster.borrow());
