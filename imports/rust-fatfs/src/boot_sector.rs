...
pub(crate) struct BiosParameterBlock {
    ...
    pub(crate) root_dir_first_cluster: RefCell<u32>,
...
impl BiosParameterBlock {
    fn deserialize<T: Read>(rdr: &mut T) -> io::Result<BiosParameterBlock> {
        ...
        if bpb.is_fat32() {
            ...
            bpb.root_dir_first_cluster = rdr.read_u32::<LittleEndian>()?.into();
    ...
    fn serialize<T: Write>(&self, mut wrt: T) -> io::Result<()> {
        ...
        if self.is_fat32() {
            ...
            wrt.write_u32::<LittleEndian>(*self.root_dir_first_cluster.borrow())?;
    ...
    fn validate(&self) -> io::Result<()> {
        ...
        if is_fat32 && self.backup_boot_sector() >= self.reserved_sectors() {
            return Err(Error::new(
                ErrorKind::Other,
                "Invalid BPB (backup boot-sector not in a reserved region)",
            ));
        }
    ...
    pub(crate) fn mirroring_enabled(&self) -> bool { self.extended_flags & 0x80 == 0 }

    pub(crate) fn active_fat(&self) -> u16 {
        // The zero-based number of the active FAT is only valid if mirroring is disabled.
        if self.mirroring_enabled() { 0 } else { self.extended_flags & 0x0F }
    }

    pub(crate) fn status_flags(&self) -> FsStatusFlags { FsStatusFlags::decode(self.reserved_1) }
    ...
    pub(crate) fn sectors_per_fat(&self) -> u32 {
        if self.is_fat32() { self.sectors_per_fat_32 } else { self.sectors_per_fat_16 as u32 }
    }

    pub(crate) fn total_sectors(&self) -> u32 {
        if self.total_sectors_16 == 0 { self.total_sectors_32 } else { self.total_sectors_16 as u32 }
    }

    pub(crate) fn reserved_sectors(&self) -> u32 { self.reserved_sectors as u32 }
    ...
    pub(crate) fn sectors_per_all_fats(&self) -> u32 { self.fats as u32 * self.sectors_per_fat() }
    ...
    pub(crate) fn fs_info_sector(&self) -> u32 { self.fs_info_sector as u32 }

    pub(crate) fn backup_boot_sector(&self) -> u32 { self.backup_boot_sector as u32 }
}
...
fn determine_sectors_per_fat(
    ...
    //
    // Note: when computing number of clusters from number of sectors rounding down is used because partial
    ...
    // sectors_per_fat * bits_per_sector >= (floor(data_sectors / sectors_per_cluster) + 2) *
    // bits_per_fat_entry
    //
    ...
    // sectors_per_fat * bits_per_sector >= ((t0 - fats * sectors_per_fat) / sectors_per_cluster + 2) *
    // bits_per_fat_entry sectors_per_fat * bits_per_sector / bits_per_fat_entry >= (t0 - fats *
    // sectors_per_fat) / sectors_per_cluster + 2 sectors_per_fat * bits_per_sector / bits_per_fat_entry
    // >= t0 / sectors_per_cluster + 2 - fats * sectors_per_fat / sectors_per_cluster sectors_per_fat *
    // bits_per_sector / bits_per_fat_entry + fats * sectors_per_fat / sectors_per_cluster >= t0 /
    // sectors_per_cluster + 2 sectors_per_fat * (bits_per_sector / bits_per_fat_entry + fats /
    // sectors_per_cluster) >= t0 / sectors_per_cluster + 2 sectors_per_fat >= (t0 / sectors_per_cluster +
    // 2) / (bits_per_sector / bits_per_fat_entry + fats / sectors_per_cluster)
    //
    // Note: MS specification omits the constant 2 in calculations. This library is taking a better
    // approach...
    //
    // sectors_per_fat >= ((t0 + 2 * sectors_per_cluster) / sectors_per_cluster) / (bits_per_sector /
    // bits_per_fat_entry + fats / sectors_per_cluster) sectors_per_fat >= (t0 + 2 * sectors_per_cluster)
    // / (sectors_per_cluster * bits_per_sector / bits_per_fat_entry + fats)
    //
...
fn try_fs_geometry(
    ...
    // This implementation uses only 8. This is enough to fit in two boot sectors (main and backup) with
    // additional bootstrap code and one FSInfo sector. It also makes FAT alligned to 4096 which is a nice
    // number.
    let reserved_sectors: u16 = if fat_type == FatType::Fat32 { 8 } else { 1 };

    // Check if volume has enough space to accomodate reserved sectors, FAT, root directory and some data
    // space Having less than 8 sectors for FAT and data would make a little sense
...
fn format_bpb(
    ...
    let bpb = BiosParameterBlock {
        ...
        fs_version: 0,
        root_dir_first_cluster: RefCell::new(if is_fat32 { 2 } else { 0 }),
...
pub(crate) fn format_boot_sector(
    ...
    let boot_code: [u8; 129] = [
        ,
    ];
