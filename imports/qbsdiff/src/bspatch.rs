...
///         .buffer_size(4096)
///         .delta_min(1024)
///         .apply(io::Cursor::new(source), io::Cursor::new(&mut target))?;
///     Ok(target)
/// }
/// ```
///
...
///     patcher.apply(io::Cursor::new(source), &mut target_file)
/// }
/// ```
...
impl<'p> Bspatch<'p> {
    ...
    pub fn new(patch: &'p [u8]) -> Result<Self> {
        Ok(Bspatch { patch: parse(patch)?, buffer_size: BUFFER_SIZE, delta_min: DELTA_MIN })
    }
    ...
    ///
    /// ~~Parameter `source` is designed to be a low-level `&[u8]` binary, rather than a `Seek + Read` random
    /// accessing data. See [#8](https://github.com/hucsmn/qbsdiff/pull/8), allowing random accessible source file might be misleading,
    /// which could result in unwanted performance loss due to heavy syscall overheads of `seek` + `read` (by
    /// the way, `pread`/`read_at` is not cross-platform)~~.
    ///
    ...
    ///
    /// For the purposes of KeyOS (the firmware update procedure, specifically) we had to resort to
    /// using a `Read + Seek` input because loading (certain) entire files into memory was hitting
    /// memory limits of the device.
    ///
    /// The target data size would be returned if no error occurs.
    pub fn apply<S: Read + Seek, T: Write>(self, source: S, target: T) -> Result<u64> {
...
/// Bspatch context.
struct Context<'p, S: Read + Seek, T: Write> {
    source: S,
    target: T,
...
impl<'p, S: Read + Seek, T: Write> Context<'p, S, T> {
    /// Create context.
    ...
    fn seek(&mut self, offset: i64) -> Result<()> { self.source.seek(SeekFrom::Current(offset)).map(drop) }
}
...
mod tests {
    ...
    fn bspatch_with_file_as_input() {
        ...
        let mut target = Vec::new();
        let _ = bspatch.apply(source_file, &mut target).unwrap();
