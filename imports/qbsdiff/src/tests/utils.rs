//! An internal module that provides helpers for testing.
...
impl Default for QbsdiffOptions {
    fn default() -> Self {
        QbsdiffOptions {
            chunk_size: 0,
            small_match: crate::bsdiff::SMALL_MATCH,
            compression_level: crate::bsdiff::COMPRESSION_LEVEL,
            buffer_size: crate::bsdiff::BUFFER_SIZE,
        }
    }
}
...
impl Default for QbspatchOptions {
    fn default() -> Self {
        QbspatchOptions { buffer_size: crate::bspatch::BUFFER_SIZE, delta_min: crate::bspatch::DELTA_MIN }
    }
}
...
impl Testing {
    ...
    pub fn new(assets_dir: path::PathBuf) -> Self { Testing { assets_dir } }

    /// Execute bsdiff command.
    pub fn _bsdiff(&self, s: &[u8], t: &[u8]) -> io::Result<Vec<u8>> {
        let dir = self.assets_dir.join("bin");
        _run_bsdiff_in(dir, s, t)
    }
    ...
    pub fn _qbspatch_with(&self, s: &[u8], p: &[u8], opts: QbspatchOptions) -> io::Result<Vec<u8>> {
        let patcher = Bspatch::new(p)?;
        let mut t = Vec::with_capacity(patcher.hint_target_size() as usize);
        patcher
            .buffer_size(opts.buffer_size)
...
impl Benchmarking {
    ...
    pub fn new(assets_dir: path::PathBuf) -> Self { Benchmarking { assets_dir } }

    /// Perform qbspatch via internal library calls.
    pub fn qbsdiff(&self, s: &[u8], t: &[u8]) -> io::Result<()> {
        Bsdiff::new(s, t).parallel_scheme(ParallelScheme::Auto).compare(io::sink())?;
        Ok(())
    }
    ...
    fn should_use_bsdiff(&self, sample: &Sample) -> bool {
        sample.patch.parent().and_then(|p| p.file_name()).map(|d| d == "samples").unwrap_or(false)
    }
}

fn _run_bsdiff_in<P: AsRef<Path>>(dir: P, s: &[u8], t: &[u8]) -> io::Result<Vec<u8>> {
    let spath = create_temp(s)?;
    let tpath = create_temp(t)?;
    let ppath = create_temp(b"")?;
    run_command_in(dir, "bsdiff", &[spath.as_os_str(), tpath.as_os_str(), ppath.as_os_str()])?;
    fs::read(ppath)
}
...
fn run_bspatch_in<P: AsRef<Path>>(dir: P, s: &[u8], p: &[u8]) -> io::Result<Vec<u8>> {
    ...
    run_command_in(dir, "bspatch", &[spath.as_os_str(), tpath.as_os_str(), ppath.as_os_str()])?;
    fs::read(tpath)
}
...
impl Sample {
    /// Load source data.
    pub fn load_source(&self) -> io::Result<Vec<u8>> { Ok(fs::read(self.source.as_path())?) }

    /// Load target data.
    pub fn load_target(&self) -> io::Result<Vec<u8>> { Ok(fs::read(self.target.as_path())?) }
}
...
fn get_samples_in<P: AsRef<Path>>(dir: P) -> io::Result<Vec<Sample>> {
    ...
    for result in walker.into_iter() {
        ...
        samples.push(Sample { name, source, target, patch });
    }
...
pub fn default_random_samples() -> Vec<RandomSample> {
    ...
    vec![
        RandomSample { name: "empty", source: SBytes(b""), targets: vec![TBytes(b""), TBytes(b"extra")] },
        RandomSample {
            name: "small",
            source: SBytes(
...
pub fn _default_random_bench_samples() -> Vec<RandomSample> {
...
fn distort(source: &[u8], similar: f64) -> Vec<u8> {
    ...
    let tsize = random_between((source.len() as f64 * 0.75) as usize, (source.len() as f64 * 1.25) as usize);
...
fn random_decide(rate: f64) -> bool { random_between(0.0, 1.0) <= fraction(rate) }
...
fn convex_mapping(frac: f64) -> f64 { (1.0 - (1.0 - frac) * (1.0 - frac)).sqrt() }
