        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) => {
                ...
                match tag_name {
                    ...
                    "bitWidth" => bit_width = Some(parse_usize(extract_contents(reader)?.as_bytes())?),
                    "bitOffset" => bit_offset = Some(parse_usize(extract_contents(reader)?.as_bytes())?),
                    _ => (),
                }
            }
            Ok(Event::End(ref e)) => {
            ...
            Ok(Event::Start(ref e)) => match e.local_name().as_ref() {
                b"peripheral" => {
                    let base_peripheral = match e.attributes().next() {
                        Some(Ok(Attribute { key: QName(b"derivedFrom"), value })) => {
...
"####;
    ...
    writeln!(out, "  /// Memory regions by name")?;
    writeln!(out, "  pub const MEMORY_REGIONS: &[(&str, core::ops::Range<usize>)] = &[")?;
    for region in regions {
        writeln!(
            out,
            "    (\"{}\", 0x{:08x}..0x{:08x}), // Size: 0x{:0x} bytes",
            region.name,
            region.base,
    ...
    writeln!(out, "  /// Peripherals by name")?;
    writeln!(out, "  pub const PERIPHERALS: &[(&str, core::ops::Range<usize>)] = &[")?;
    for peripheral in peripherals {
        let size = estimate_peripheral_size(peripheral);
        writeln!(
            out,
            "    (\"{}\", 0x{:08x}..0x{:08x}), // Size: 0x{:0x} bytes",
            peripheral.name,
...
pub fn generate<T: AsRef<Path>, U: Write>(src: T, dest: &mut U) -> Result<(), ParseError> {
    ...
    print_memory_map(&description.memory_regions, &description.peripherals, dest)
